 ...
 */
export function getPriceIdForPlan(plan, billingCycle = 'monthly') {
 ...
 */
export async function canAddLinks(userId, currentLinksCount) {
  const subscription = await prisma.subscription.findUnique({
    where: { userId },
  })
 ...
 */
export function getLinksLimit(plan) {
 ...
 */
export async function canUseTheme(userId, themeName) {
  const subscription = await prisma.subscription.findUnique({
    where: { userId },
  })
...
/**
 * Downgrada usuário para plano FREE
 */
 ...
 */
export async function updateSubscriptionInDb(customerId, updates) {
